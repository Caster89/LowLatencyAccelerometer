...
//buttons
enum keyNameE: byte{noneBtn=0, upBtn=1,downBtn=2,leftBtn=3,rightBtn=4,selectBtn=5};
//expected deviation in btn voltage
#define maxDeviation 5
//latency between double button press
...
byte getKey(){
  ...
  if(abs(currValue-928)<maxDeviation)
    {return upBtn;}
  else if(abs(currValue-902)<maxDeviation)
    {return downBtn;}
  else if(abs(currValue-855)<maxDeviation)
    {return leftBtn;}
  else if(abs(currValue-815)<maxDeviation)
    {return rightBtn;}
  else if(abs(currValue-615)<maxDeviation)
    {return selectBtn;}
  else
    {return noneBtn;}
}
...
//------------------------------------------------------------------------------
void recordBinFile() {
  Serial.print(F("Starting the recording"));
...
{
  Serial.print(F("Starting off by looking at file:"));
  Serial.print(csvName);
  uint8_t position = 4;
  while(sd.exists(csvName) && position>=0) {
    Serial.print(F("File exists"));
    if(csvName[position] != '9'){
      csvName[position]++;
    } else{
      csvName[position] ='0';
      if(position>0){
  ...
  Serial.print(F("Settled for file name:"));
  Serial.print(csvName);
}
//-----------------------------------------------------------------------------
void renameBinFile() {
  /*uint8_t position = 4;
  while(sd.exists(binName) && position>=0) {
    if(binName[position] != '9'){
      binName[position]++;
    } else{
      binName[position] ='0';
      if(position>0){
  ...
  if (!binFile.rename(sd.vwd(), csvName)) {
...
void loop(void) {
  ...
  byte startMeasure = 0;
  Serial.print(F("Waiting for start signal"));
  while (startMeasure==0){
    byte i=getKey();
    if(i==selectBtn && (millis()-lastPressTime)>disableDelay){
      Serial.print(F("Select button pushed"));
      delay(1);
      startMeasure=1;
